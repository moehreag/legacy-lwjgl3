...
public final class SDLDisplay implements Display.Impl {
	@NotNull
	...
	private ByteBuffer @Nullable [] cached_icons = null;
	...
	private final SDL_DropEvent dropEvent = event.drop();
	...
	public static SDLDisplay getInstance() {
		return (SDLDisplay) Display.impl;
	}
		...
		while (SDL_PollEvent(event)) {
			switch (event.type()) {
				...
				case SDL_EVENT_MOUSE_BUTTON_DOWN, SDL_EVENT_MOUSE_BUTTON_UP, SDL_EVENT_MOUSE_MOTION,
					...
					if (Mouse.isCreated()) {
						((SDLMouseImplementation) LWJGLImplementationUtils._mouseImplementation).processMouseEvent(event);
					}
				}
	...
	public void checkSdlError(boolean success) {
		if (!success) {
	...
	public void setFullscreen(boolean fullscreen) {
		...
		try {
			...
			LegacyLWJGL3.LOGGER.warn("Failed to set fullscreen status: ", t);
		}
	}
	...
	class NoOpImpl implements Drawable {
		...
		public void makeCurrent() throws LWJGLException {
			if (isCreated()) {
				prevHandle = getHandle();
				Display.create();
			}
			SDLDisplay.this.makeCurrent();
		}

		@Override
		public void releaseContext() {
			SDLVideo.SDL_GL_MakeCurrent(getHandle(), 0);
		}

		@Override
		public void destroy() {
			SDLVideo.SDL_DestroyWindow(getHandle());
			if (prevHandle != -1L) {
