...
public class GLFWKeyboardImplementation implements KeyboardImplementation {
	private final List<NativeResource> callbacks = new ArrayList<>(4);
	...
	public void createKeyboard() {
		...
		var charCallback = GLFWCharCallback.create((window, codepoint) -> {
			if (MinecraftAccess.getInstance().screen == null) {
				// without a screen there also isn't anywhere to input text.
				// As such, we can safely skip these events which could cause IMEs to pop up while in-game, this would also prevent actual
				// keybinds from triggering correctly
				return;
			}
		...
		var imeStatusCallback = GLFWIMEStatusCallback.create(window -> IMEManager.getInstance().notifyIMEChanged());
		var imePreeditCallback = GLFWPreeditCallback.create((window, preedit_count, preedit_string, block_count, block_sizes, focused_block, caret) ->
				IMEManager.getInstance().submitPreeditEvent(PreeditEvent.createFromCallback(preedit_count, preedit_string, block_count, block_sizes, focused_block, caret)));
		callbacks.add(imePreeditCallback);
		callbacks.add(imeStatusCallback);
		...
		GLFW.glfwSetIMEStatusCallback(windowHandle, imeStatusCallback);
		GLFW.glfwSetPreeditCallback(windowHandle, imePreeditCallback);
	}
	...
	public void destroyKeyboard() {
		callbacks.forEach(NativeResource::free);
	}
